...
def buildNetwork(cfg):
    ...
    while n_placed < cfg.network.n_nodes:
        ...
        if i>=cfg.network.n_iter:
            #raise Exception("Valid point not found within {} iterations".format(cfg.network.n_iter))
            #print("Valid point not found within {} iterations".format(cfg.network.n_iter))
            break
